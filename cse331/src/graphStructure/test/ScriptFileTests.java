...
/**
 * This class, along with a complete HW5TestDriver implementation, can be used
 * to test the your implementations of Graph and the path finding algorithm
 * using the script file format described in the problem set. It is assumed that
 * the files are located in the same directory as this class.
 *
 ...
 * and for each of those it will run the checkAgainstExpectedOutput() test. See
 * the JUnit4 Javadocs for more information, or Google for more examples.
 */
...
public class ScriptFileTests {
    ...
    public static void calculateTestFiles() {
        ...
        try {
            ...
            File myDirectory = new File(ScriptFileTests.class.getResource("ScriptFileTests.class").toURI())
                    .getParentFile();
            for (File f : myDirectory.listFiles(ScriptFileTests.testFileFilter)) {
                ScriptFileTests.testScriptNames.add(f.getName());
                ScriptFileTests.testScriptFiles.add(f);
            }
    ...
    public static List<Object[]> getTestFiles() {
        ...
        // a list of arg array.
        List<Object[]> filesToTest = new ArrayList<Object[]>(ScriptFileTests.testScriptFiles.size());
    ...
    private String runScriptFile() throws IOException {
        if (this.testScriptFile == null) {
            throw new RuntimeException("No file specified");
        }

        File actual = this.fileWithSuffix("actual");

        Reader r = new FileReader(this.testScriptFile);
        Writer w = new FileWriter(actual);

        TestDriver td = new TestDriver(r, w);
        td.runTests();

        return this.fileContents(actual);
    }
    ...
    private File fileWithSuffix(String newSuffix) {
        File parent = this.testScriptFile.getParentFile();
        String driverName = this.testScriptFile.getName();
    ...
    public void checkAgainstExpectedOutput() throws IOException {
        ...
        File expected = this.fileWithSuffix("expected");
        Assert.assertEquals(this.testScriptFile.getName(), this.fileContents(expected), this.runScriptFile());
    }
}
